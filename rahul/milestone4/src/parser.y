...
%type <node> LocalVariableDeclaratorList LocalVariableDeclarator LocalVariableDeclaratorId LocalVariableInitializer
%type <node> MemberName FormalParameterList FormalParameter ThisAccess
...
%type <node> ForUpdate StatementExpressionList  BreakStatement PrintStatement ClassType
    ...
    : MULTI_ClassModifier Type VariableDeclaratorList ';' {
			...
			fori(0, siz){
				...
				if(t == "None"){
					...
					if(t != $2->place){
						cerr << "Incompatible type in declaration at line: " << line << "\n";
						exit(1);
					}
				}
				$$->varDecs[i]->type = t;
...
LocalVariableDeclaration
    : Type LocalVariableDeclaratorList            { 
		$$ = $2;
		...
		$$->type = typeName;
		if($$->type != "None" && equal_compatible($$->type, $1->type) == "None"){
			// cerr<<$2->type <<$1->type << endl;
			cerr << "Incompatible types in declaration at line: " << line << endl;
			// exit(1);
		}
    }
...
LocalVariableDeclaratorList  //TODO : NEED SOME CHANGES
...
LocalVariableDeclarator 
    : LocalVariableDeclaratorId '=' LocalVariableInitializer {
        genNode* newNode = new genNode();
			$$ = newNode;
...
LocalVariableInitializer //TODO : METHOD DECLARATION
   : Expression { $$=$1;}
   | ArrayInitialize { $$=$1;}
   ;
...
StatementWithoutTrailingSubstatement //TODO: start from here tomorrow
    : Block                                  { $$ = $1; }
    | EmptyStatement                         { $$ = $1; }
