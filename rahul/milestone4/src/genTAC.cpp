...
void setLineNum(genNode *node)
{
	...
	fori(0, siz)
	{
		t = node->code[i];
		t->lineNum = i + 1;
		if (isOpIn(op3, 21, t->op))
			t->opType = 3;
		else if (isOpIn(op2, 14, t->op))
			t->opType = 2;
		else if (isOpIn(op1, 10, t->op))
			t->opType = 1;
		else if (isOpIn(op0, 4, t->op))
		{
			t->opType = -1;
		}
	}
...
void printTAC(genNode *node)
{
	...
	fori(0, siz)
	{
		...
		if (t == NULL)
		{
			output << "line number %d has the tac class empty\n";
			exit(1);
		}
		// output << t->op << "\t\t";
		switch (t->opType)
		...
		case -1:
			...
			{
					// output<< t->op << " " <<t->oldScope->name <<" "<< t->newScope->name;
					output << t->op << " " << t->oldScope->name << " " << t->newScope->name;
			}
			else
			{
...
void getCECode(genNode *d, genNode *c, genNode *s1, genNode *s2, int lineNum)
{
	...
	tac1->op = "label";
	tac1->target = newLabel1;
	tac2->op = "label";
	tac2->target = newLabel2;
	tac3->op = "ifgoto";
	tac3->dest = ST->GetVar(c->place);
	tac3->target = newLabel1;
	...
	tac4->op = "goto";
	tac4->target = newLabel2;
	tac5->op = "goto";
	tac5->target = newLabel3;
	tac6->op = "label";
	tac6->target = newLabel3;
	tac7->op = "=";
	tac7->dest = temp;
	if (s1->isLit)
	{
